...
<html>
    ...
    <body bgcolor=#E0EBEA>
        <%
            ...
            String SqlQuery = "SELECT   x.aa, x.cem, z.account_title"
                    + " FROM   ( select COALESCE (bal1,bal2) aa, cem from ("
                    + " select s1.acct_no_cr bal1, s2.acct_no_dt bal2, s1.cem cem1,"
                    + " s2.cem cem2, (nvl(s2.cem,0)-nvl(s1.cem,0)) cem  from"
                    + " (SELECT  acct_no_cr, sum(tr_amount_lcy_dt) cem"
                    + " FROM   si_transaction_account_un"
                    + " WHERE (gl_acct_no_cr LIKE ('6%') or gl_acct_no_cr LIKE ('7%'))"
                    + " AND gl_acct_no_dt='50130'"
                    + " AND COD_PLAT like ('%" + filial + "%') "
                    + "  AND (entry_date between is_opdate_next(to_date('" + strDateB + "','yyyy-mm-dd')) and is_opdate(to_date('" + strDateE + "','yyyy-mm-dd')))"
                    + " group by acct_no_cr) s1,"
                    + " (SELECT acct_no_dt, sum(tr_amount_lcy_dt) cem"
                    + " FROM si_transaction_account_un"
                    + " WHERE  (gl_acct_no_dt LIKE ('6%') or gl_acct_no_dt LIKE ('7%'))"
                    + " AND gl_acct_no_cr ='50130'"
                    + " AND COD_PLAT like ('%" + filial + "%') "
                    + " AND (entry_date between is_opdate_next(to_date('" + strDateB + "','yyyy-mm-dd')) and is_opdate(to_date('" + strDateE + "','yyyy-mm-dd')))"
                    + " group by acct_no_dt) s2 "
                    + " where s1.acct_no_cr =  s2.acct_no_dt(+)"
                    + " union "
                    + " select s1.acct_no_cr bal1, s2.acct_no_dt bal2, s1.cem cem1, s2.cem cem2, (nvl(s2.cem,0)-nvl(s1.cem,0)) cem  from"
                    + " (SELECT acct_no_cr, sum(tr_amount_lcy_dt) cem"
                    + " FROM si_transaction_account_un"
                    + " WHERE (gl_acct_no_cr LIKE ('6%') or gl_acct_no_cr LIKE ('7%'))"
                    + " AND gl_acct_no_dt='50130'"
                    ...
                    + " AND (entry_date between is_opdate_next(to_date('" + strDateB + "','yyyy-mm-dd')) and is_opdate(to_date('" + strDateE + "','yyyy-mm-dd')))"
                    + " group by acct_no_dt) s2"
                    + " where s1.acct_no_cr(+) =  s2.acct_no_dt)) x,"
                    + " si_account_inf_bal z WHERE x.aa = z.alt_acct_id(+) and z.date_until='01-JAN-3000'";

            // System.out.println(SqlQuery);    
            ResultSet sqlSel = stmt.executeQuery(SqlQuery);

            String SqlQuery2 = "SELECT   x.aa, x.cem, z.account_title"
                    + " FROM   ( select COALESCE (bal1,bal2) aa, cem from ("
                    + " select s1.acct_no_cr bal1, s2.acct_no_dt bal2, s1.cem cem1,"
                    ...
                    + " where s1.acct_no_cr =  s2.acct_no_dt(+)"
                    + " union "
                    + " select s1.acct_no_cr bal1, s2.acct_no_dt bal2, s1.cem cem1, s2.cem cem2, (nvl(s1.cem,0)-nvl(s2.cem,0)) cem  from"
                    + " (SELECT acct_no_cr, sum(tr_amount_lcy_dt) cem"
                    + " FROM si_transaction_account_un"
                    + " WHERE (gl_acct_no_cr LIKE ('8%') or gl_acct_no_cr LIKE ('9%'))"
                    + " AND gl_acct_no_dt='50130'"
            ...
            String SqlQuery3 = "SELECT nvl(sum(a.balance_lcy_amount),0) cem"
                    ...
                    + "  AND a.date_from <=is_opdate (TO_DATE ('" + strDateB + "','yyyy-mm-dd')-1)"
                    + " and a.date_until >= is_opdate (TO_DATE ('" + strDateB + "', 'yyyy-mm-dd')-1)"
            ...
            SqlQuery3 = "SELECT nvl(sum(a.balance_lcy_amount),0) cem"
                    ...
                    + " where a.gl_acct_no = '50130' "
                    + " AND b.date_until='01-JAN-3000' and b.alt_acct_id=a.alt_acct_id"
                    + "  AND a.date_from <=is_opdate (TO_DATE ('" + strDateE + "','yyyy-mm-dd')-1)"
                    + " and a.date_until >= is_opdate (TO_DATE ('" + strDateE + "', 'yyyy-mm-dd')-1)"
                    + " AND b.filial_code like ('%" + filial + "%')";
