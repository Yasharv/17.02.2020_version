...
/**
 * Wrapper for the central directory entry (CDE) of one file. At the end of the
 * ZIP file one CDE can be found for each entry (file) within the zip file.
 *
 * central file header signature 4 bytes (0x02014b50) version made by 2 bytes	//
 * 4 version needed to extract 2 bytes //	6 general purpose bit flag 2 bytes //
 * 8 compression method 2 bytes // 10 last mod file time 2 bytes	// 12 last mod
 * file date 2 bytes	// 14 crc-32 4 bytes	// 16 compressed size 4 bytes	// 20
 ...
 *
 * file name (variable size)	// 46 extra field (variable size)	// 46 +
 * fileNameLength file comment (variable size)	// 46 + fileNameLength +
 * extraFieldLength
 */
public class CentralDirectoryEntry implements ZipConstants {
    ...
    /**
     * @return position within zip file where the actual data of the entry
     * (file) starts (after encryption salt + pw data)
     */
    ...
    /**
     * (for encrypted files) stored in extra field java zip only supports STORED
     * and DEFLATED
     *
